						...
						<table class="table table-striped table-hover">
							...
							<c:forEach items="${order}" var="order">
								<c:url var="up" value="OrderServlet">
									...
									<c:param name="userID" value="${order.user_id}" />
								</c:url>
								...
								<tr>
									...
									<td><a href="${up}"><button>修改</button></a></td>
									<td><a onclick="if( !(confirm('確認刪除?') ) ) return false"
										href="${de}"><button>刪除</button></a></td>
								</tr>
							</c:forEach>
							...
							<c:forEach items="${order}" var="order">
								...
								<tr>
									...
									<c:if test="${order.status == 0}">
										<td><a href="${upStatus}"><button>結帳</button></a></td>
									</c:if>
									<c:if test="${order.status == 1}">
										<td><a href="${upStatus}"><button id="btn" disabled>待審核</button></a></td>
									</c:if>
									<c:if test="${order.status == 2}">
										<td><a href="${up}"><button style="background-color: blue;">詳細</button></a></td>
									</c:if>
